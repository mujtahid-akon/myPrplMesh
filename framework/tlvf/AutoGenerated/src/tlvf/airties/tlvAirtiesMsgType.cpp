...
tlvAirtiesMsgType::tlvAirtiesMsgType(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
tlvAirtiesMsgType::tlvAirtiesMsgType(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
tlvAirtiesMsgType::~tlvAirtiesMsgType() {
}
const eAirtiesTlvTypeMap& tlvAirtiesMsgType::type() {
    return (const eAirtiesTlvTypeMap&)(*m_type);
}

const uint16_t& tlvAirtiesMsgType::length() {
    return (const uint16_t&)(*m_length);
}

sVendorOUI& tlvAirtiesMsgType::vendor_oui() {
    return (sVendorOUI&)(*m_vendor_oui);
}

void tlvAirtiesMsgType::class_swap()
{
    tlvf_swap(16, reinterpret_cast<uint8_t*>(m_length));
    m_vendor_oui->struct_swap();
}

bool tlvAirtiesMsgType::finalize()
{
    if (m_parse__) {
...
size_t tlvAirtiesMsgType::get_initial_size()
{
    size_t class_size = 0;
